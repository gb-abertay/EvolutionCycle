...
void AAntPlusReaderActor::BeginPlay()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("APR: Ant+ Initial Values Set"));
}
...
{
    UE_LOG(LogTemp, Warning, TEXT("APR: Closing channel..."));
    bBroadcasting = FALSE;
    pclMessageObject->CloseChannel(type + 1, MESSAGE_TIMEOUT);
}

bool AAntPlusReaderActor::SetChannelID(int DevID, int DevType, int TransType, DSISerialGeneric* pclSO, DSIFramerANT* pclMO)
{
    UE_LOG(LogTemp, Warning, TEXT("APR: Initialising Channel ID and required pointers to run"));
    ...
    if (bStatus)
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: Reader Set Up With Channel ID %i/%i/%i"), DeviceNumber, DeviceType, TransmissionType);
        (new FAutoDeleteAsyncTask<WaitForMessagesTask2>(this, pclMessageObject, type))->StartBackgroundTask();
        return true;
    }
    else
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: Failed to set up channel of ID %i/%i/%i"), DeviceNumber, DeviceType, TransmissionType);
        return false;
    }
}

void AAntPlusReaderActor::ProcessMessage(ANT_MESSAGE stMessage, USHORT usSize_, int DevType)
{
    BOOL bStatus;
    ...
    case MESG_RESPONSE_EVENT_ID: //Channel Message (Channel Response/Event)
    {
        ...
        if (stMessage.aucData[0] == (type + 1))
        {
            ...
            case MESG_NETWORK_KEY_ID: //Config Messages (Set Network Key)
            {
                ...
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error configuring network key: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Network key set."));
                UE_LOG(LogTemp, Warning, TEXT("APR: Assigning channel..."));
                bStatus = pclMessageObject->AssignChannel(type + 1, 0, 0, MESSAGE_TIMEOUT);
                break;
            }
            ...
            case MESG_ASSIGN_CHANNEL_ID: //Config Messages (Assign Channel)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error assigning channel: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Channel assigned"));
                UE_LOG(LogTemp, Warning, TEXT("APR: Setting Channel ID..."));
                bStatus = pclMessageObject->SetChannelID(type + 1, DeviceNumber, DeviceType, TransmissionType, MESSAGE_TIMEOUT);
                break;
            }
            ...
            case MESG_CHANNEL_ID_ID: //Config Messages (Channel ID)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error configuring Channel ID: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Channel ID set"));
                UE_LOG(LogTemp, Warning, TEXT("APR: Setting Radio Frequency..."));
                bStatus = pclMessageObject->SetChannelRFFrequency(type + 1, USER_RADIOFREQ, MESSAGE_TIMEOUT);
                break;
            }
            ...
            case MESG_CHANNEL_RADIO_FREQ_ID: //Config Messages (Channel RF Frequency)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error configuring Radio Frequency: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Radio Frequency set"));
                UE_LOG(LogTemp, Warning, TEXT("APR: Setting Channel Period..."));
                bStatus = pclMessageObject->SetChannelPeriod(type + 1, 8182, MESSAGE_TIMEOUT);
                break;
            }
            ...
            case MESG_CHANNEL_MESG_PERIOD_ID: //Config Messages (Channel Period)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error configuring Channel Period: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Channel Period set"));
                UE_LOG(LogTemp, Warning, TEXT("APR: Opening channel..."));
                bBroadcasting = TRUE;
                bStatus = pclMessageObject->OpenChannel(type + 1, MESSAGE_TIMEOUT);
                break;
            }
            ...
            case MESG_OPEN_CHANNEL_ID: //Control Messages (Open Channel)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error opening channel: Code 0%d"), stMessage.aucData[2]);
                    bBroadcasting = FALSE;
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Channel opened"));
                ...
                UE_LOG(LogTemp, Warning, TEXT("APR: Enabling extended messages..."));
            ...
            case MESG_ANTLIB_CONFIG_ID: //Config Messages (Lib Config)
            {
                ...
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Extended messages not supported in this ANT product"));
                    break;
                }
                else if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error enabling extended messages: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Extended messages enabled"));
                break;
            }
            ...
            case MESG_UNASSIGN_CHANNEL_ID: //Config Messages (Unassign Channel)
            {
                if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error unassigning channel: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
                UE_LOG(LogTemp, Warning, TEXT("APR: Channel unassigned"));
                return;
                break;
            }
            ...
            case MESG_CLOSE_CHANNEL_ID: //Control Messages (Close Channel)
            {
                if (stMessage.aucData[2] == CHANNEL_IN_WRONG_STATE)
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("APR: Channel is already closed"));
                    UE_LOG(LogTemp, Warning, TEXT("APR: Unassigning channel..."));
                    bStatus = pclMessageObject->UnAssignChannel(type + 1, MESSAGE_TIMEOUT);
                    break;
                }
                else if (stMessage.aucData[2] != RESPONSE_NO_ERROR)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Error closing channel: Code 0%d"), stMessage.aucData[2]);
                    break;
                }
            ...
            case MESG_REQUEST_ID: //Control Messages (Request Message)
            {
                if (stMessage.aucData[2] == INVALID_MESSAGE)
                {
                    UE_LOG(LogTemp, Warning, TEXT("APR: Requested message not supported in this ANT product"));
                }
                break;
            }
            ...
            case MESG_EVENT_ID: //Channel Event (NOT CHANNEL RESPONSE)
            {
                ...
                case EVENT_CHANNEL_CLOSED:
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("APR: Channel Closed"));
                    UE_LOG(LogTemp, Warning, TEXT("APR: Unassigning channel..."));
                    bStatus = pclMessageObject->UnAssignChannel(type + 1, MESSAGE_TIMEOUT);
                    break;
                }
                case EVENT_TX:
                {
                    ...
                    if (bBroadcasting)
                    {
                        pclMessageObject->SendBroadcastData(type + 1, aucTransmitBuffer);

                        // Echo what the data will be over the air on the next message period.
                        if (bDisplay)
                        {
                            UE_LOG(LogTemp, Warning, TEXT("APR: Echo what the data will be over the air on the next message period."));
                            UE_LOG(LogTemp, Warning, TEXT("APR: Tx:(%d): [%02x],[%02x],[%02x],[%02x],[%02x],[%02x],[%02x],[%02x]"),
                                type + 1,
                        ...
                        else
                        {
                            static int iIndex = 0;
                            static char ac[] = { '|', '/', '-', '\\' };
                            UE_LOG(LogTemp, Warning, TEXT("APR: Tx: %c\r"), ac[iIndex++]);
                            fflush(stdout);
                            iIndex &= 3;
                        }
                    }
                ...
                case EVENT_RX_SEARCH_TIMEOUT:
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("APR: Search Timeout"));
                    break;
                }
                case EVENT_RX_FAIL:
                {
                ...
                case EVENT_TRANSFER_TX_COMPLETED:
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("APR: Tranfer Completed"));
                    break;
                }
                ...
                case EVENT_TRANSFER_TX_START:
                {
                    ...
                    sent to the device. */
                    UE_LOG(LogTemp, Warning, TEXT("APR: Burst Started"));
                    break;
                }
                default:
                {
                    /*Wtf happened lol*/
                    UE_LOG(LogTemp, Warning, TEXT("APR: Unhandled channel event: 0x%X"), stMessage.aucData[2]);
                    break;
                }
            ...
            default:
            {
                UE_LOG(LogTemp, Warning, TEXT("APR: Unhandled response 0%d to message 0x%X"), stMessage.aucData[2], stMessage.aucData[1]);
                break;
            }
            }
            break;
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: RESET Complete, reason: "));

        UCHAR ucReason = stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX];

        if (ucReason == RESET_POR)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_POR"));
        if (ucReason & RESET_SUSPEND)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_SUSPEND "));
        if (ucReason & RESET_SYNC)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_SYNC "));
        if (ucReason & RESET_CMD)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_CMD "));
        if (ucReason & RESET_WDT)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_WDT "));
        if (ucReason & RESET_RST)
            UE_LOG(LogTemp, Warning, TEXT("APR: RESET_RST "));
        UE_LOG(LogTemp, Warning, TEXT("APR: "));

        break;
    }
    case MESG_CAPABILITIES_ID: //Requested Response Message (Capabilities)
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES:"));
        UE_LOG(LogTemp, Warning, TEXT("APR:    Max ANT Channels: %d"), stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX]);
        UE_LOG(LogTemp, Warning, TEXT("APR:    Max ANT Networks: %d"), stMessage.aucData[MESSAGE_BUFFER_DATA2_INDEX]);
        ...
        UE_LOG(LogTemp, Warning, TEXT("APR: Standard Options:"));
        if (ucStandardOptions & CAPABILITIES_NO_RX_CHANNELS)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_RX_CHANNELS"));
        if (ucStandardOptions & CAPABILITIES_NO_TX_CHANNELS)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_TX_CHANNELS"));
        if (ucStandardOptions & CAPABILITIES_NO_RX_MESSAGES)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_RX_MESSAGES"));
        if (ucStandardOptions & CAPABILITIES_NO_TX_MESSAGES)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_TX_MESSAGES"));
        if (ucStandardOptions & CAPABILITIES_NO_ACKD_MESSAGES)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_ACKD_MESSAGES"));
        if (ucStandardOptions & CAPABILITIES_NO_BURST_TRANSFER)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NO_BURST_TRANSFER"));

        UE_LOG(LogTemp, Warning, TEXT("APR: Advanced Options:"));
        if (ucAdvanced & CAPABILITIES_OVERUN_UNDERRUN)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_OVERUN_UNDERRUN"));
        if (ucAdvanced & CAPABILITIES_NETWORK_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_NETWORK_ENABLED"));
        if (ucAdvanced & CAPABILITIES_AP1_VERSION_2)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_AP1_VERSION_2"));
        if (ucAdvanced & CAPABILITIES_SERIAL_NUMBER_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_SERIAL_NUMBER_ENABLED"));
        if (ucAdvanced & CAPABILITIES_PER_CHANNEL_TX_POWER_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_PER_CHANNEL_TX_POWER_ENABLED"));
        if (ucAdvanced & CAPABILITIES_LOW_PRIORITY_SEARCH_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_LOW_PRIORITY_SEARCH_ENABLED"));
        if (ucAdvanced & CAPABILITIES_SCRIPT_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_SCRIPT_ENABLED"));
        if (ucAdvanced & CAPABILITIES_SEARCH_LIST_ENABLED)
            UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_SEARCH_LIST_ENABLED"));

        if (usSize_ > 4)
        {
            UE_LOG(LogTemp, Warning, TEXT("APR: Advanced 2 Options 1:"));
            if (ucAdvanced2 & CAPABILITIES_LED_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_LED_ENABLED"));
            if (ucAdvanced2 & CAPABILITIES_EXT_MESSAGE_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_EXT_MESSAGE_ENABLED"));
            if (ucAdvanced2 & CAPABILITIES_SCAN_MODE_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_SCAN_MODE_ENABLED"));
            if (ucAdvanced2 & CAPABILITIES_RESERVED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_RESERVED"));
            if (ucAdvanced2 & CAPABILITIES_PROX_SEARCH_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_PROX_SEARCH_ENABLED"));
            if (ucAdvanced2 & CAPABILITIES_EXT_ASSIGN_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_EXT_ASSIGN_ENABLED"));
            if (ucAdvanced2 & CAPABILITIES_FS_ANTFS_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_FREE_1"));
            if (ucAdvanced2 & CAPABILITIES_FIT1_ENABLED)
                UE_LOG(LogTemp, Warning, TEXT("APR: CAPABILITIES_FIT1_ENABLED"));
        }
        break;
    }
    case MESG_CHANNEL_STATUS_ID: //Requested Response Message (Channel Status)
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: Got Status"));
        ...
        UE_LOG(LogTemp, Warning, TEXT("APR: STATUS: %s"), astrStatus[ucStatusByte]);
        break;
    }
    case MESG_CHANNEL_ID_ID: //Requested Response Message (Channel ID)
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: CHANNEL ID: (%d/%d/%d)"), DeviceNumber, DeviceType, TransmissionType);
        break;
    }
    case MESG_VERSION_ID: //Requested Response Message (ANT Version)
    {
        UE_LOG(LogTemp, Warning, TEXT("APR: VERSION: %s"), (char*)&stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX]);
        break;
    }
    ...
    case MESG_BROADCAST_DATA_ID:
    {
        ...
        if (usSize_ > MESG_DATA_SIZE)
        {
            ...
            if (bDisplay && ucFlag & ANT_EXT_MESG_BITFIELD_DEVICE_ID)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("APR: Chan ID(%d/%d/%d) - "), usDeviceNumber, ucDeviceType, ucTransmissionType);
            }
        }
        ...
        if (bDisplay)
        {
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("APR: Acked Rx:(%d): "), stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX]);
            }
            else if (stMessage.ucMessageID == MESG_BURST_DATA_ID)
            {
                UE_LOG(LogTemp, Warning, TEXT("APR: Burst(0x%02x) Rx:(%d): "), ((stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX] & 0xE0) >> 5), stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX] & 0x1F);
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("APR: Rx:(%d): "), stMessage.aucData[MESSAGE_BUFFER_DATA1_INDEX]);
            }
        }
        break;
    }
    ...
    if (bPrintBuffer)
    {
        if (bDisplay)
        {
            UE_LOG(LogTemp, Warning, TEXT("APR: [%02x],[%02x],[%02x],[%02x],[%02x],[%02x],[%02x],[%02x]"),
        ...
        else
        {
            static int iIndex = 0;
            static char ac[] = { '|', '/', '-', '\\' };
            UE_LOG(LogTemp, Warning, TEXT("APR: Rx: %c\r"), ac[iIndex++]); 
            fflush(stdout);
            iIndex &= 3;
...
WaitForMessagesTask2::WaitForMessagesTask2(AAntPlusReaderActor* APRActor, DSIFramerANT* pclMsgObj, int DevType)
{
    AntPlusReaderActor = APRActor;
...
{
    UE_LOG(LogTemp, Warning, TEXT("APR: Stopped Waiting For Messages!!"))
}

void WaitForMessagesTask2::DoWork()
{
    UE_LOG(LogTemp, Warning, TEXT("APR: Thread Started"))
        while (true)
        {
            ...
            if (pclMessageObject->WaitForMessage(1000))
            {
                ...
                if (usSize == DSI_FRAMER_ERROR)
                {
                    ...
                    UE_LOG(LogTemp, Warning, TEXT("APR: thread error"));
                }

                else if (usSize != DSI_FRAMER_TIMEDOUT && usSize != 0)
                {
                    AntPlusReaderActor->ProcessMessage(stMessage, usSize, DeviceType);
                    UE_LOG(LogTemp, Warning, TEXT("APR: processing messages"));
                }
            }
        }
}